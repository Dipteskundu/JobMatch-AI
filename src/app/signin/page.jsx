...
export default function SignInPage() {
  ...
  async function syncUserWithBackend(user) {
    try {
      // First, fetch existing user profile to get their role
      let existingRole = "candidate"; // default fallback
      try {
        const profileRes = await fetch(
          `${apiBase}/api/auth/profile/${user.uid}`,
        );
        if (profileRes.ok) {
        ...
        console.log("Could not fetch existing profile, using default role");
      }
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-slate-900 relative overflow-hidden flex-col justify-between p-16 animate-slide-in-left">
        ...
        <div className="relative z-10">
          ...
          <p className="text-slate-400 text-lg max-w-md leading-relaxed">
            ...
            and manage your professional profile.
          </p>
        </div>
        ...
        <div className="relative z-10 flex flex-col gap-8">
          ...
          <p className="text-slate-500 text-sm font-medium">
            Join 50,000+ professionals finding their dream roles.
          </p>
        </div>

    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col justify-center items-center p-6 animate-fade-in">
        <div className="bg-white p-10 rounded-[2.5rem] shadow-xl border border-slate-100 max-w-md text-center transform transition-all hover:scale-105 duration-300">
            ...
            <h1 className="text-2xl font-black text-slate-800 mb-4">Saved Jobs</h1>
            <p className="text-slate-500 font-medium mb-8">Please sign in to view your saved jobs and track your career interests.</p>
            <Link href="/signin" className="block w-full bg-indigo-600 text-white py-4 rounded-2xl font-black hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200 active:scale-95">
                Sign In Now
            </Link>
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 pt-24 md:pt-32">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6 animate-fade-up">
          <div>
            <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-black text-indigo-600 uppercase tracking-widest">Your Private Collection</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight leading-tight">
              Saved <span className="text-indigo-600">Jobs</span>
            </h1>
            <p className="text-slate-500 font-medium mt-2 max-w-xl">
              Keep track of roles that interest you. Apply whenever you&apos;re ready or review them later.
            </p>
          </div>
          
          <Link href="/jobs" className="flex items-center gap-2.5 bg-white border border-slate-200 text-slate-700 px-6 py-3.5 rounded-2xl font-black shadow-sm hover:border-indigo-200 hover:text-indigo-600 transition-all active:scale-95 hover:shadow-lg">
        ...
        <div className="space-y-6 reveal">
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {savedJobs.map((job) => (
                <div key={job._id} className="group relative bg-white rounded-[2rem] border border-slate-100 p-8 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300">
            ...
            <div className="bg-white rounded-[3rem] border-2 border-dashed border-slate-100 p-20 text-center">
              ...
              <Link href="/jobs" className="inline-flex items-center gap-2 bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all active:scale-95">
                Start Searching
                <ChevronRight className="w-4 h-4" />
              </Link>
            </div>
